module game {
	 ...
	 *
	 */
	 export class CTween extends egret.EventDispatcher {
		/**
         * 不做特殊处理
         ...
         * @platform Web,Native
		 */
        public static get(target: any,props: any = null,pluginData: any = null,override: boolean = false): CTween {
            if (override) {
        ...
        public static removeTweens(target:any):void {
            ...
            var tweens: CTween[] = CTween._tweens;
         ...
         * @param value
         */
        private static _register(tween: CTween, value:boolean):void {
         ...
         * @param paused
         */
        private static onTick(): boolean {
            let timeStamp = game.TimeUtil.getTimeStatmp();
            var delta = timeStamp - CTween._lastTime;
            CTween._lastTime = timeStamp;

            var tweens: CTween[] = CTween._tweens.concat();
            for(var i = tweens.length - 1;i >= 0;i--) {
                var tween: CTween = tweens[i];
                if(tween.paused) {
                    continue;
                }
         ...
         * @platform Web,Native
		 */
        public setPaused(value: boolean): CTween {
            this.paused = value;
        ...
        private _appendQueueProps(o):any {
            ...
            for (var m in o) {
                oldValue = this._curQueueProps[m];
                if(arr = CTween._plugins[m]) {
                    injectProps = injectProps || {};
                    for (i = 0, l = arr.length; i < l; i++) {
                        if (arr[i].step) {
                            arr[i].step(this, m, oldValue, o[m], injectProps);
                        }
                    }
                }
                this._curQueueProps[m] = o[m];
            }
            if (injectProps) {
         ...
         * @platform Web,Native
		 */
        public wait(duration: number,passive?: boolean): CTween {
         ...
         * @platform Web,Native
		 */
        public to(props: any,duration?: number,ease: Function = undefined): CTween {
         ...
         * </pre>
		 */
        public call(callback: Function,thisObj: any = undefined,params: Array<any> = undefined): CTween {
         ...
         * @platform Web,Native
         */
        public set(props: any,target = null): CTween {
         ...
         * @platform Web,Native
		 */
        public play(tween?: CTween): CTween {
            if (!tween) {
                tween = this;
            }
         ...
         * @platform Web,Native
		 */
        public pause(tween?: CTween): CTween {
            if (!tween) {
                tween = this;
            }
